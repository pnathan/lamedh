...
- **Lint**: `cargo clippy --workspace --all-targets --features fuzz -- -D warnings` (warnings are errors, matching CI and the gauntlet; `--features fuzz` so the fuzz battery in `tests/brutal_correctness.rs` is linted too, since that file compiles under no other gate)
- **Format**: `cargo fmt --all`
- **Benchmarks**: `cd benchmarks && ./run_benchmarks.sh`

Run `cargo fmt --all` and `cargo clippy --workspace --all-targets --features fuzz -- -D warnings` before every commit; clippy warnings are errors — CI and the gauntlet both enforce `-D warnings`.
...
- `46-hm-check.lisp`: the **portable Hindley-Milner checker** (#451) — the checking half of `src/jit/{infer,types,elaboration,registry}.rs` ported to Lamedh so every host runs the same checking logic instead of reimplementing it natively, plus (section 7b) the typing half of the codegen mode — the compileable-type gate — that `47-typed-island.lisp` builds on. Owns the type vocabulary, unification (row polymorphism, nominal-into-row subsumption, variant absorption), generalize/instantiate, the declaration registry, and a bidirectional elaborator over real Lamedh surface syntax. Entry points: `hm-see-type`, `hm-verdict`, `hm-check-lambda`, `hm-check-expr`, `hm-audit`. Verdicts are recomputed, never cached: a verdict is derived from the whole world (callee bodies, declared axioms, the registries) and not every definition path can be hooked, so a cache would go stale and report a confident wrong answer. **Loads early** (right after `21-cl-compat.lisp`, before `20-condensation.lisp` — `STDLIB_SOURCES`' order is explicit, not numeric) because it wraps `declare-type!`/`record-declare`/`variant-declare`/`declare-instance!`/`declare-protocol-dispatch!` so `defrecord`, `defvariant`, `definstance` and `28-types.lisp` feed the portable registry from their existing call sites. `defun`'s `$defun-auto-compile` calls its `$hm-on-defun` hook on every definition; `condense-verdict` (in `20-condensation.lisp`) is the seam that makes the whole condensation honesty machinery run off the native checker here and off this one on a host without one.
- `47-typed-island.lisp`: the **typed-island front end** (`docs/typed-island-frontend.md`), built on `46-hm-check.lisp`'s codegen-mode gate (section 7b: the portable `checking: false` elaborator plus `Infer::resolve`; section 10: `hm-compile-verdict`, `hm-compile-group`). `typed-island` takes a list of names and returns a manifest — the largest group that compiles together under the gate, every member with a monomorphic signature over the compileable lattice and a frozen (macro-expanded) body, closed under calls, with every rejection carrying the kernel's own blocker wording. `island-optimize` re-gates optimized bodies and reports type-changing optimizations instead of taking them; `island-forms` is the manifest as `declare-typed` + `defun-typed` forms; `island-install!` hands it to the host kernel and reads `see-type` back (`AGREE`/`DISAGREE`/`KERNEL-REJECTED` per member); guarded by default (a non-fitting call falls back to the dynamic closure), `'strict` binds the kernel's entry as `defun-typed` does. Author annotations (`defun-typed`, typed `defun*`) enter the group as pins. Loads last among the libraries (it needs `optimize-form` and the module system).
...
- Run `cargo clippy --workspace --all-targets --features fuzz -- -D warnings`; warnings are errors. Note any environment-only failures.
