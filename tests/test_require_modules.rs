...
fn with_stdlib_still_loads_every_optional_module_and_marks_it_loaded() {
    ...
    // #260 os/os-linux modules, the #365 tls module, and the #408 regex
    // module. (#451's 45-hm-check.lisp is NOT here: it is a core,
    // load-order-critical file -- it installs the declaration-plane wrappers
    // every later library's declare-type!/record-declare calls go through --
    // so it is unconditionally part of STDLIB_SOURCES, never requirable.)
    assert_eq!(line(&env, "(length (loaded-modules))"), "32");
}
