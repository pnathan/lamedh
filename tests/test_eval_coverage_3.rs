...
fn test_get_non_hashtable_error() {
    let env = env_with_stdlib();
    let result = eval_line("(get 42 \"k\")", &env);
    assert!(result.contains("Error") || result.contains("error"),
        "expected error for (get 42 \"k\"), got: {result}");
}
...
fn test_make_hash_table_and_set_get() {
    ...
    let result = eval_line(
        "(progn (def h (make-hash-table)) (set-bang h \"key\" \"val\") (get h \"key\"))",
        &env,
    );
    assert!(!result.contains("Error"),
...
fn test_delete_key_bang_removes_entry() {
    let env = env_with_stdlib();
    let result = eval_line(
        "(progn (def h (make-hash-table)) (set-bang h 'x 99) (delete-key-bang h 'x) (get h 'x))",
        &env,
    );
