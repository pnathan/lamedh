...
(macrolet ((wrap (name fn) `(defbuiltin ,name (&rest args) (apply ,fn args))))
  (wrap "+" #'+) (wrap "-" #'-) (wrap "*" #'*) (wrap "/" #'/)
...
(defbuiltin "DIFFERENCE" (a b) (- a b))
(defbuiltin "QUOTIENT" (a b) (/ a b))
(defbuiltin "MOD" (a b) (mod a b))
(defbuiltin "REMAINDER" (a b) (rem a b))
(defbuiltin "EXPT" (a b) (expt a b))
(defbuiltin "ZEROP" (x) (bool (zerop x)))
(defbuiltin "EVENP" (x) (bool (evenp x)))
(defbuiltin "ODDP" (x) (bool (oddp x)))
(defbuiltin "PLUSP" (x) (bool (plusp x)))
(defbuiltin "ADD1" (x) (+ x 1))
(defbuiltin "SUB1" (x) (- x 1))
(env-set-local *global-env* (lsym "1+") (lambda (x) (+ x 1)))
(env-set-local *global-env* (lsym "1-") (lambda (x) (- x 1)))
...
(defbuiltin "ROUND" (x &optional (y 1)) (values (round x y)))
...
(defbuiltin "SUBSTRING" (s start end) (subseq (->str s) start end))
(defbuiltin "CHAR-CODE" (c) (char-code (if (characterp c) c (char (->str c) 0))))
(defbuiltin "CODE-CHAR" (n) (code-char n))
(defbuiltin "MAKE-CHAR" (n) (code-char n))
(defbuiltin "STRING-TO-NUMBER" (s)
...
(defbuiltin "NUMBER-TO-STRING" (n) (lprint-to-string n nil))
...
(defbuiltin "ERROR" (msg &rest more)
  (lamedh-error (if more (format nil "~A ~{~A~^ ~}" (->str msg) (mapcar #'lprint-to-string more)) (->str msg))))
