...
/// which allocated **two** `Vec`s per call — one holding the (refcount-bumped)
/// argument *expressions*, one holding the evaluated results. Walking the cons
/// cells and evaluating each `car` in place does the same work with a single
/// allocation: it drops the intermediate operand `Vec` and the extra pass over
/// it. (Like the cons-walking special forms `AND`/`OR`/`PROGN`, an improper
/// tail is ignored.)
            ...
            if let LispVal::Symbol(s) = &**first {
                match s.borrow().name.as_str() {
                    ...
                    _ => {
                        ...
                        // Macros receive UNEVALUATED operands, so materialize the
                        // operand list only on this (comparatively cold) path.
                        ...
                        // Evaluated-argument callables (lambda/builtin/native):
                        // evaluate operands straight off the cons chain — one
                        // allocation, no intermediate list of cloned expressions.
