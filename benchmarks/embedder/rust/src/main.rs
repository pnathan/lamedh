...
fn main() {
    lamedh::with_large_stack(|| {
        ...
        let a1 = time(
            "A1 eval_str (string+parse+eval — the game today)",
            200_000,
            || {
        ...
        let a3 = time(
            "A3 typed registry jit_call (NATIVE defun*)",
            2_000_000,
            || {
                black_box(
                    env.jit_call("TICK-T", &[Value::Float(0.016_666)])
                        .unwrap()
                        .unwrap(),
                );
            },
        );
        let jh = env.jit_fn_handle("TICK-T").expect("typed fn");
        let a3_5 = time(
            "A3.5 JitFnHandle (pinned id, scalar fast path)",
            5_000_000,
            || {
                black_box(
                    env.jit_call_handle(&jh, &[Value::Float(0.016_666)])
                        .unwrap(),
                );
            },
        );
        ...
        println!(
            "   ratios vs Rust: eval_str {:.0}x, preparsed {:.0}x, call_function {:.0}x, FnHandle {:.0}x, jit_call {:.0}x, jit_handle {:.0}x\n",
            a1 / a4,
            a2 / a4,
            a2_5 / a4,
            a2_6 / a4,
        ...
        println!(
            "B1 interpreted lisp loop + interpreted kernel      {b1:>12.1} ns/eval   ({v1:?})"
        );
        ...
        println!(
            "B2 per-sample jit_call into NATIVE defun*          {b2:>12.1} ns/eval   (sum {s:.1})"
        );
        ...
        println!(
            "B2.5 per-sample raw native entry (#424)            {b2_5:>12.1} ns/eval   (sum {s25:.1})"
        );

        let t0 = Instant::now();
        let v3 = env
            .jit_call("KERNEL-LOOP-T", &[Value::Int(n as i64)])
            .unwrap()
            .unwrap();
        let b3 = t0.elapsed().as_secs_f64() * 1e9 / n as f64;
        println!(
            "B3 whole loop NATIVE (one jit_call)                {b3:>12.1} ns/eval   ({v3:?})"
        );
        ...
        println!(
            "B4 pure Rust loop                                  {b4:>12.1} ns/eval   (sum {s4:.1})"
        );
        println!(
        ...
        println!(
            "C1 defun* NATIVE dot over typed arrays             {c1:>12.1} ns/elem   ({})",
        ...
        println!(
            "C2 pure Rust iterator dot                          {c2:>12.1} ns/elem   ({dot:.1})"
        );
